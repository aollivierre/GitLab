        ...
        Switch ($Item) {
            ...
            Exchange {
                If ($MFA -eq $True) {
                    ...
                    # $MFAExchangeModule = (Get-ChildItem $env:LOCALAPPDATA"\Apps\2.0\" -Include CreateExoPSSession.ps1, Microsoft.Exchange.Management.ExoPowershellModule.dll -Recurse | Group-Object Directory | Where-Object { $_.Count -eq 2 }).Values | Sort-Object LastWriteTime -Descending | Select-Object -First 1 | Select-Object -ExpandProperty FullName
                    
                    # $UserProfile = ""
                    # $UserProfile = $Env:USERPROFILE

                    $Path = ""
                    $Path = "$PSScriptRoot\Microsoft Exchange Online Powershell Module\"
                    # $Path = "$UserProfile\GitHub\Office365\O365\Connect-O365ServicesMFA\Microsoft Exchange Online Powershell Module\"
                    
                    $MFAExchangeModule = @()
                    $MFAExchangeModule = (Get-ChildItem $Path -Include CreateExoPSSession.ps1, Microsoft.Exchange.Management.ExoPowershellModule.dll -Recurse | Group-Object Directory | Where-Object { $_.Count -eq 2 }).Values | Sort-Object LastWriteTime -Descending | Select-Object -First 1 | Select-Object -ExpandProperty FullName
